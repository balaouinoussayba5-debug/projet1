    ...
    if (isset($_POST['reset'])) {
        ...
        if (!$result['success']) {
            ...
            if (!empty($result['correct'])) {
                ...
                $message = $result['message'] . ' (le nombre était ' . htmlspecialchars(debug_target()) . ')';
            }
        }
    }
}
...
<pre class="small">
Target (secret) : <?= htmlspecialchars(debug_target()) . PHP_EOL ?>
Essais utilisés : <?= htmlspecialchars($attempts_used) ?> / <?= htmlspecialchars($_SESSION['max_attempts']) . PHP_EOL ?>
Essais restants : <?= htmlspecialchars($attempts_remaining) ?>
</pre>
</div>
</body>
</html>